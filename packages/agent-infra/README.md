...
descriptor (`agent.yaml`, via `@leanish/agent-runtime`'s `loadDescriptorFromFile`) plus the
...
`need` produces — is specified at `../../specs/agentic-development/agent-infra/specs/contract.md`.
...
2. **Packages must be publishable** — `@leanish/agent-runtime` / `@leanish/agent-atc` are still
