...
## Posture
- **Implemented and tested**; infra provisions the scheduler tick + self-publish wiring, only the Dockerfile is still deferred — see `SCOPE.md` + `ASSUMPTIONS.md`.
...
- Integration: `test-integration/` runs the real Lambda entry against LocalStack (`npm run test:integration`; requires `docker compose up -d localstack`). Fake runner only — never a live coding agent, never GitHub writes.
- `npm run check` must stay green; run `npm run check:full` when touching the lambda entry, handler stages, or self-publish behavior.
