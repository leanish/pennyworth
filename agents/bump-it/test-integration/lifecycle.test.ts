...
/**
 * Full three-stage bump-it lifecycle against real LocalStack resources:
 *
 ...
 *   real DynamoDB, syncs a REAL git clone, runs the (fake) `bump-it`
 ...
 */
describe("bump-it lifecycle end-to-end against LocalStack", () => {
  ...
  it("walks init → breakdown → revisit chain with the revisit cap terminating the loop", async () => {
    ...
    const ctx = await provisionBumpItStack(stack, [optedIn, notOptedIn]);
    ...
    ctx.fakeRunner.register("bump-it", () =>
      fencedJson({
        summary:
        ...
        pullRequests: [
          {
            ...
            branch: "bump-it/dependency-refresh",
            number: 41,
          ...
          },
          {
            ...
            branch: "bump-it/GHSA-aaaa-bbbb-cccc",
            number: 42,
    ...
    expect(branches).toEqual(["bump-it/GHSA-aaaa-bbbb-cccc", "bump-it/dependency-refresh"]);

    // ---- stage 3: revisit round 0 → deferred + reschedule (count 1) ----
    const batchedRevisit = afterBreakdown.find(
      (s) => s.body.payload["branch"] === "bump-it/dependency-refresh",
    )!;
    ctx.fakeRunner.register("bump-it-revisit", () =>
      fencedJson({
        outcome: "deferred",
    ...
    expect(revisitInvocation.entrypoint.name).toBe("bump-it-revisit");
    ...
    expect(round1.body.payload["branch"]).toBe("bump-it/dependency-refresh");
    ...
    expect(round2.body.payload["branch"]).toBe("bump-it/dependency-refresh");
    ...
    const cveRevisit = afterBreakdown.find(
      (s) => s.body.payload["branch"] === "bump-it/GHSA-aaaa-bbbb-cccc",
    )!;
  ...
  it("dedupes a redelivered breakdown via the real DynamoDB claim — the skill runs once", async () => {
    ...
    const ctx = await provisionBumpItStack(stack, [project]);
    ctx.fakeRunner.register("bump-it", () =>
