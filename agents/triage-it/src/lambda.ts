...
/**
 * triage-it's AWS Lambda entry module. The exported `handler` is the
 * function registered with the Lambda runtime; AWS event source mapping
 * for the input SQS queue routes records to it.
 *
 ...
 *
 * No self-publisher is wired — triage-it has a single `init` stage and no
 * fan-out.
 *
 ...
 *   CONSUMER_REGISTRY_TABLE_NAME   — DynamoDB consumer-registry table.
 ...
 *   EVENT_BUS_NAME                 — EventBridge custom bus for lifecycle events.
 *   WORKSPACE_ROOT                 — optional, defaults to `/tmp/triage-it-workspaces`.
 *   TRIAGE_IT_TMP_DIR              — optional. Base dir for evidence-archive
 *                                    extraction; defaults to `os.tmpdir()`.
 *   TRIAGE_IT_SIGNING_KEY_TTL_MS   — optional. TTL for cached SSM signing keys.
 ...
 */
export interface CreateTriageItLambdaOptions {
...
export type TriageItLambdaHandler = (event: SqsEvent) => Promise<SqsBatchResponse>;

/**
 * Construct the Lambda handler. Most production code uses the cached
 * `handler` export below; this factory is the testable seam.
 */
export async function createTriageItLambdaHandler(
  options: CreateTriageItLambdaOptions = {},
): Promise<TriageItLambdaHandler> {
  ...
  const workspaceRoot = process.env["WORKSPACE_ROOT"] ?? "/tmp/triage-it-workspaces";
  ...
  // The agent's own skills/ directory sits next to its agent.yaml; the
  // runtime's bundled skills/ is the fallback for shared support skills
  // (`karpathy-guidelines`).
  ...
  const s3 = new AwsS3Client({
    ...
    // override is safe to enable whenever an endpoint override is in play.
  ...
  const signingKeyTtlMs = parseOptionalInt("TRIAGE_IT_SIGNING_KEY_TTL_MS");
 ...
 *
 *   export const handler = triageItLambdaHandler;
 *
 * is the canonical Lambda registration in the infra package.
 */
let cachedHandlerPromise: Promise<TriageItLambdaHandler> | undefined;

export const triageItLambdaHandler: TriageItLambdaHandler = async (event) => {
  cachedHandlerPromise ??= createTriageItLambdaHandler();
