...
- `cli.md` — `add`, `discover`, drafting prompt, flags, error model.
- `data-format.md` — spine + slug patterns, filename rule.
- `0006-tech-stack.md` — deps + the gh-shell-out / own-invoker / prompt-lib decisions.
...
- **No-agent-runtime boundary preserved:** the invoker, gh wrapper, and clone manager are catalogit-local; nothing imports `@leanish/runtime`.
