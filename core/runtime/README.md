...
- `../../agents/ask-the-code/` — ATC Q&A backend (uses `@leanish/runtime`); package `@leanish/ask-the-code`.
- `../../agents/secure-it/` — phase-2 dep-upgrade agent (uses `@leanish/runtime`).
...
- Sub-entrypoints: `@leanish/runtime/local` (local-mode adapters bundle) and `@leanish/runtime/testing` (in-memory fakes for downstream agent tests).
