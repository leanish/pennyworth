...
describe("agent registry", () => {
  ...
  it("registers the bump-it agent with a descriptor path + ECR repo", () => {
    const bumpIt = AGENTS.find((a) => a.id === "bump-it");
    expect(bumpIt).toBeDefined();
    expect(bumpIt?.descriptorPath).toMatch(/bump-it\/agent\.yaml$/);
    expect(bumpIt?.ecrRepositoryName).toBe("leanish/agent-bump-it");
    expect(bumpIt?.imageTag.length).toBeGreaterThan(0);
  });
